...
function TodoList({todo, _id}) {
  ...
  return (
    ...
    className={`${
        ...
        px-2 py-2 cursor-pointer flex items-center justify-between`}>{todo}
    <span onClick={()=> dispatch(deleteTodos(_id)) } className='text-xl text-gray-300 hover:text-red-500 duration-300 cursor-pointer'><MdDelete /></span></motion.li>
  )
}
